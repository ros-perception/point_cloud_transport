...
#ifndef POINT_CLOUD_TRANSPORT__TL__EXPECTED_HPP_
#define POINT_CLOUD_TRANSPORT__TL__EXPECTED_HPP_
...
#if defined(_MSC_VER) && _MSC_VER <= 1900
// TODO(anyone) make a version which works with MSVC 2015
...
std::true_type can_swap(int) noexcept(noexcept(swap(std::declval<T &>(), std::declval<U &>())));  // NOLINT
...
struct expected_storage_base
{
  ...
  template<class ... Args,
    ...
    nullptr>
  constexpr explicit expected_storage_base(in_place_t, Args &&... args)
...
struct expected_storage_base<void, E, false, true>
{
  ...
  constexpr explicit expected_storage_base(in_place_t)
  : m_has_val(true) {}
...
public:
  ...
  and_then(F && f) &->decltype(and_then_impl(
  ...
  template<class F>
  constexpr decltype(map_error_impl(
    std::declval<const expected &>(),
    std::declval<F &&>()))
  map_error(F && f) const & {
    return map_error_impl(*this, std::forward<F>(f));
  ...
  constexpr explicit expected(unexpected<G> const & e)
  : impl_base(unexpect, e.value()),
    ctor_base(detail::default_constructor_tag{}) {}
...
#endif  // POINT_CLOUD_TRANSPORT__TL__EXPECTED_HPP_
